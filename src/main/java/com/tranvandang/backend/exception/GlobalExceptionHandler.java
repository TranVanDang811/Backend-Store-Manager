...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiResponse<Object>> handlingValidation(MethodArgumentNotValidException exception) {
        ...
        try {
            ...
            if (!errors.isEmpty()) { // Kiểm tra tránh lỗi IndexOutOfBoundsException
                var constraintViolation =
                        errors.get(0).unwrap(ConstraintViolation.class); // ✅ Lấy phần tử đầu tiên đúng cách

                attributes =
                        constraintViolation.getConstraintDescriptor().getAttributes(); // ✅ Đảm bảo phương thức hợp lệ
